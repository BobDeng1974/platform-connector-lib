...
func initKafkaProducer(zkurl string) sarama.AsyncProducer {
	var kz *kazoo.Kazoo
...
func (this *Connector) produceKafka(topic string, message string) {
	if this.Config.LogProduce == "true"{
		log.Println("Produce", topic, message)
	}
	this.kafkaproducer.Input() <- &sarama.ProducerMessage{Topic: topic, Key: nil, Value: sarama.StringEncoder(message), Timestamp: time.Now()}
}
